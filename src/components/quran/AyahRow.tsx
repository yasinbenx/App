    ...
    <div className="border-b border-sand-200/70 py-6 first:pt-2 last:border-b-0">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <AyahPlayButton surahId={surahId} ayahNumber={ayah.number} size="sm" />
        </div>
        <button
          type="button"
        ...
        >
          <Bookmark size={18} strokeWidth={2} className={isBookmarked ? 'fill-gold-400 text-gold-500' : ''} />
        </button>
      </div>
      ...
      {showTranslation && <p className="text-[14.5px] leading-relaxed text-ink-700">{ayah.translationDe}</p>}
    </div>
  );
}
